...
function renderSlot(slot, index, bonuses) {
  ...
  return `<article class="slot-card ${friend ? "friend" : ""} ${mismatch ? "invalid-grand" : ""}" data-slot="${index}" data-friend-drop-target="true">
    ...
    ${renderSlotRecommendation(index)}
    ${ownFields}
    <div class="slot-bottom">${servant ? `<div class="slot-cost">${friend ? "好友位 Cost 不计入总计" : `本位 Cost ${formatNumber(slotCost)}`}${grandServant ? " · 常规礼装计入；冠位礼装不计 Cost" : ""}${completeProfile ? "" : " · 正在补全数据"}</div>` : ""}${friend ? "" : (mismatch ? '<div class="slot-result"><div class="empty-result warning">冠位模式下不可编入</div></div>' : (calculation ? renderResult(profile, calculation) : ""))}</div>
  </article>`;
}
